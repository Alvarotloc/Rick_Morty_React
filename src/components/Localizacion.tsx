...
const Localizacion = ({dimension,name,type,residents}:iLocalizacion):JSX.Element => {
    const [personajes,setPersonajes]:[personajes:(Personaje | Personaje[]),setPersonajes:Function] = useState([]);
  ...
  return (
    <>
        <section className="w-full border-2 border-black bg-gray-900/30 rounded-lg flex flex-col transition-all">
            ...
            <h4 className="text-white font-semibold ml-5 text-lg">Dimension: {dimension}</h4>
            <h4 className="text-white font-semibold ml-5 my-5 text-lg">Tipo: {type}</h4>
            <h4 className={visible ? "text-white font-semibold ml-5 mb-5 text-lg" : "hidden"}>Residentes: {visible && personajes.length > 1 ? personajes.map((personaje:Personaje,index:number) => personaje.name + (personajes.length - 1 !== index ? ", " : ".")) : (personajes as any).name + "."}</h4>
